...
div(v-if="loading")
  div(class="flex justify-center items-center h-40")
    div(class="w-10 h-10 border-4 border-black border-t-black rounded-full animate-spin")
  ...
  div(class="px-4 py-8 max-w-7xl mx-auto")
    ...
    div(class="flex justify-between mb-6 flex-wrap gap-4")
      div(class="flex items-center gap-2")
        button(@click="toggleFilter" class="bg-black border px-4 py-2 rounded flex items-center gap-2")
          font-awesome-icon (:icon="['fas', 'sliders']" class="text-xl")
          span  Filter Price
        div(v-if="showFilter" class="absolute bg-black shadow rounded mt-2 z-50")
          div(v-for="option in priceOptions" :key="option.value" @click="selectPrice(option.value)" class="p-2 hover:bg-black cursor-pointer")
            span {{ option.label }}

      div
        select(v-model="categoryFilter" class="border px-4 py-2 rounded")
          option(value="All Categories") All Categories
          option(v-for="category in uniqueCategories" :key="category.id" :value="category.name" class="bg-black") {{ category.name }}
    ...
    div(class="flex justify-center gap-2 mt-8")
      button(v-for="page in totalPages" :key="page" @click="changePage(page)" :class="['px-4 py-2 rounded border', currentPage === page ? 'bg-white text-black' : 'bg-black hover:bg-white hover:text-black transition']") {{ page }}
</template>
